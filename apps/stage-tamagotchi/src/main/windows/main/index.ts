...
export async function setupMainWindow(params: {
  ...
  const mainWindowConfig = getConfig().windows?.find(w => w.title === 'EVA' && w.tag === 'main')
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => w.title === 'EVA' && w.tag === 'main')
    ...
    if (existingConfigIndex === -1) {
      ...
      const mainWindowConfig = defu(config.windows[existingConfigIndex], { title: 'EVA', tag: 'main' })
